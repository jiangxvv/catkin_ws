...
DPtug::DPtug(const std::vector<double>& init_velocity,
           ...
           const std::vector<double>& init_thrust)
{
  ...
  actuation_ <<init_position_[0],init_position_[2],init_position_[2];
}

// private methods

// rotation matrix defination
Eigen::Matrix3d DPtug::RotationMatrix(const Eigen::Vector3d& pose) const
{
  Eigen::Matrix3d rz;
...
Eigen::Matrix3d DPtug::InertiaMatrix() const
{
...
Eigen::Matrix3d DPtug::InertiaCCMatrix(const Eigen::Vector3d& vec) const
{
...
Eigen::Vector3d DPtug::DampingVector(const Eigen::Vector3d& vec) const
{
  double U =
...
DPtug::Vector6d DPtug::StateDerivative(const Eigen::Vector3d& velocity,
                                       const Eigen::Vector3d& position,
                                       const double) const
{
  DPtug::Vector6d state_derivative;
...
void DPtug::operator() (const std::vector<double>& state,
...
void RunVehicle(DPtug& vehicle, const size_t& step_number=600,
