...
// The class of the mothership to be towed
...
// constructor
MotherShip::MotherShip(const std::vector<double>& init_velocity,
             const std::vector<double>& init_position,
             const std::vector<double>& init_thruster)
    : init_velocity_(init_velocity), init_position_(init_position), init_thruster_(init_thruster)
{
  ...
  actuation_ << init_thruster[0], init_thruster[1], init_thruster[2] ;
}

// private methods

// rotation matrix 
inline Eigen::Matrix3d MotherShip::RotationMatrix(const Eigen::Vector3d& pose) const
{
    Eigen::Matrix3d rz;
...
inline Eigen::Matrix3d MotherShip::InertiaMatrix() const
{
...
inline Eigen::Matrix3d MotherShip::InertiaCCMatrix(const Eigen::Vector3d& vec) const
{
...
inline Eigen::Vector3d MotherShip::DampingVector(const Eigen::Vector3d& vec) const
{
  double U =
  ...
  damping1 << Xu_* vec[0], Yv_* vec[1] + Yr_* vec[2], Yr_* vec[1] + Nr_* vec[2];
...
MotherShip::Vector6d MotherShip::StateDerivative(const Eigen::Vector3d& velocity,
                                       const Eigen::Vector3d& position,
                                       const double t) const
{
    MotherShip::Vector6d state_derivative;
...
void RunVehicle(MotherShip& vehicle, const size_t& step_number=600,
